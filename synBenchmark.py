...
from RefCheck.Parser import tctl_grammar, PARSER, NUSMV
...
def run_nusmv_check(model_path: str, properties: List[str], nusmv_path: str = "/Users/brdlcu/NuSMV/bin/NuSMV") -> float:
    """
    ...
    temp_model_path = "./temp_model.smv"
...
def run_once(idx,
    ...
    time_brute_force = run_nusmv_check(model_path, all_properties["nusmv"])
...
#
    ...
    time_mc_refined = run_nusmv_check(model_path, required_properties_svm)
...
#
    ...
    tqdm.write(f"Time for Brute-Force (check all {len(all_properties)} props):      {time_brute_force:.4f} s")
    tqdm.write(f"Time with the Method (analyze + check {len(required_properties)} props): {total_time_with_analysis:.4f} s")
    ...
    if total_time_with_analysis < time_brute_force:
        ...
        tqdm.write(f"CONCLUSION: the method is FASTER by {time_brute_force - total_time_with_analysis:.4f} seconds ({speedup:.2f}x speedup).")
        diff=speedup
        ...
        tqdm.write("This may happen if the analysis time is longer than the time saved by checking fewer properties.")
        diff = -slowdown
...
def main():
    ...
    parser.add_argument("--input", type=str, default="input_benchmarks.csv", help="Input file.")
    parser.add_argument("--output", type=str, default="test_benchmark.csv", help="Result output file.")
    parser.add_argument("--output_dir", type=str, default="result/synOutput", help="Benchmark results folder.")
